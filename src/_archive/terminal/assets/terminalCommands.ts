...
export const TERMINAL_MESSAGES = {
  ...
  ACCESS_DENIED: (cmd: string) =>
    `Access denied: '${cmd}' requires authentication. Use 'login <6-digit-code>' first.`,
            ...
            if (response.ok) {
              ...
              if (result.success && result.data.authenticated) {
                ...
                const remainingMinutes = Math.floor(
                  (result.data.remaining % (1000 * 60 * 60)) / (1000 * 60),
                );
